...
public class MessageConsumer {
    ...
    public void consumeMessage(ConsumerRecord<String, MessageModel> messageModel) {
        try {
            ...
            if (!offerMessageToQueue(messageId, message)) {
                throw new BusinessException("Message queue overflow for Message: " + messageId);
            }

            processingExecutor.submit(() -> processMessages(messageModel.key(), messageModel.topic(), messageId));
        } catch (Exception e) {
